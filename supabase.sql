...
-- ======================================
-- VIEWS: haplogroup + enslaved traces
-- ======================================
...
siblings AS (
  SELECT p.id,
    TRIM(CONCAT(p.first_name, ' ', COALESCE(p.middle_name, ''), ' ', p.last_name, ' ',
        COALESCE(p.suffix, ''), COALESCE(' (' || p.other_names || ')', ''))) AS full_name,
    CASE p.gender WHEN 'Male' THEN 'Brother'
    ...
    p.dna_match
  FROM public.persons p
  INNER JOIN home h ON (p.mother_id = h.mother_id OR p.father_id = h.father_id)
                    AND p.id <> h.home_id
),
children AS (
  SELECT p.id,
...
WITH RECURSIVE descendants_tree AS (
  SELECT
    ...
    p.dna_match
  FROM public.persons p
  INNER JOIN public.persons home ON (p.mother_id = home.id OR p.father_id = home.id)
...
-- ======================================
-- RLS ENABLEMENT (ALL TABLES PUBLIC READ / AUTH WRITE)
-- ======================================
...
CREATE POLICY marriages_write ON public.marriages
  FOR INSERT, UPDATE TO authenticated
  WITH CHECK ((SELECT auth.uid()) IS NOT NULL);

CREATE POLICY dataset_write ON public.dataset
  FOR INSERT, UPDATE TO authenticated
  WITH CHECK ((SELECT auth.uid()) IS NOT NULL);
...
CREATE POLICY chat_history_write ON public.chat_history
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT auth.uid()) IS NOT NULL);
